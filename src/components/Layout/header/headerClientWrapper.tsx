...
export const HeaderClientWrapper = ({
  ...
  return (
    <header
      className={`w-[88%] md:w-[95%] lg:w-[97%] min-h-22 flex items-center justify-between m-auto overflow-hidden fixed top-0 right-0 left-0 z-50 backdrop-blur-xs transition-all duration-500 ${isHidden ? '-translate-y-full' : 'translate-y-0'}`}
    >
      {children}
    </header>
  );
