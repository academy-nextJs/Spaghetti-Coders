...
export async function DetailCommentContainer({ ID }: { ID: string }) {
  const { data: comments }: { data: CommentDataType[] } = await api.get(
    `/houses/${ID}/comments?page=1&limit=20`
  );
  return (
    <section className="max-w-full md:max-w-1/2 flex flex-col gap-12">
      <div className="flex gap-4">
        <MyChip
          size="lg"
          className="bg-transparent border border-[#7575FE] text-[#7575FE] px-3"
        >
          نظرات کاربران
        </MyChip>
        <ClientButton
          size="lg"
          className="bg-transparent text-[#7575FE] px-1 h-auto"
        >
