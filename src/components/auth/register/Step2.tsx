    ...
    <article className="w-full max-w-md flex flex-col">
      ...
      <section className="text-sm items-center justify-between mb-12">
        <p className="text-sm text-[#767676]">
          ...
          <span className="text-primaryPurple">{email}</span> را وارد کنید
        </p>
        <span
          className="text-primaryPurple underline cursor-pointer"
          onClick={() => router.back()}
        >
          تغییر ایمیل
        </span>
      ...
      <form action={formAction}>
        ...
        <div className="flex flex-row gap-1 mt-4">
          <ClientButton
            className="bg-[#7575FE30] text-primaryPurple px-2 h-10 cursor-auto"
            startContent={<HugeiconsIcon icon={TimeQuarter02Icon} />}
            disabled
            type="button"
          >
            <span className="ml-1">{formatTime(timer)}</span>
          </ClientButton>
          <span className="text-[#767676] text-sm mt-3">
        ...
        <ClientButton
          type="submit"
          className="w-full bg-primaryPurple text-white rounded-full py-3 mt-6"
          disabled={isPending}
        >
          ارسال
        </ClientButton>
